  ...
  // ------------------- Méthodes -------------------- //
    ...
    private void buildMap(String text){
      ...
      for(char c : text.toCharArray()){
        ...
        if(c == '1'){
          this.ajouter(new Texture(ApplicationInfo.getPathMedia() + "cube.jpeg", new Point(posX,posY), WidthCase,HeightCase));
          posX = posX + WidthCase;
        }

        if(c == '2'){
          avatars.add(new Avatar(ApplicationInfo.getPathMedia() + "J1.gif", new Point(posX, posY), WidthCase*2, HeightCase*2, 1));
          this.ajouter(avatars.get(0));
          posX = posX + WidthCase;
        }

        if (c =='3'){
          avatars.add(new Avatar(ApplicationInfo.getPathMedia() + "J2.gif", new Point(posX, posY), WidthCase*2, HeightCase*2, 2));
      ...
      for ( int i = 0 ; i < bullets.size() ; i++ ){
        for (int j = 0 ; j < avatars.size() ; j++){
          if(avatars.get(j).getBoiteEnglobante().intersection(bullets.get(i).getBoiteEnglobante())){
            ...
            bullets.get(i).setTransparent(true);
            bullets.remove(i); 
